...
def verify_release(root: Path, *, tag: str | None = None) -> ReleaseMetadata:
    ...
    if not isinstance(version, str) or STABLE_VERSION.fullmatch(version) is None:
        raise RuntimeError("Project version must use MAJOR.MINOR.PATCH.")

    citation = yaml.safe_load((root / "CITATION.cff").read_text(encoding="utf-8"))
    if not isinstance(citation, dict) or str(citation.get("version")) != version:
    ...
    if tag is None:
        return ReleaseMetadata(version, citation_date)
    ...
    return ReleaseMetadata(version, release_date)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify source metadata before a release."
    )
    parser.add_argument("--project-root", type=Path, default=Path.cwd())
    parser.add_argument("--tag")
    args = parser.parse_args()
    metadata = verify_release(args.project_root.resolve(), tag=args.tag)
    print(f"Verified kohdalab-iv {metadata.version} release metadata.")
    return 0
