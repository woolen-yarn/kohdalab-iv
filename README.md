...
KohdaLab IV は、Yokogawa GS210 を source、Agilent/HP 34401A、Agilent/Keysight 34411A、Keysight 34465A、または ADCMT 7461A を meter として使う DC IV/VI 測定ツールです。GUI、CLI、Jupyter Notebook から同じ測定コアを使えます。
...
ADCMT 7461A は `command_language` で SCPI/ADC を切り替えられます。標準 config は `scpi` で、connect 時の error check は `:SYSTem:ERRor?`、測定値の取得は `:READ?` を使います。SCPI setting command ごとに error check を挟みます。
...
GS210 as the source and an Agilent/HP 34401A, Agilent/Keysight 34411A, Keysight 34465A, or ADCMT 7461A as the meter.
...
The ADCMT 7461A command set is selected with `command_language`. The standard config uses `scpi`, with `:SYSTem:ERRor?` for connect-time error checks and `:READ?` for readings. SCPI setting commands are followed by an error check.
