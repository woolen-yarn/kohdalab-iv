...
ADCMT 7461A は USB/GPIB の両方で使える ADCMT command language で制御し、connect 時の error check も `ERR?` を使います。
...
The ADCMT 7461A is controlled with the ADCMT command language, including `ERR?` for connect-time error checks, so the same driver can be used over USB and GPIB.
