...
ADCMT 7461A は `command_language` で SCPI/ADC を切り替えられます。標準 config は `scpi` で、測定値の取得は `:READ?` を使います。USB connect 時は `*CLS` だけを送り、SCPI setting command ごとに error check を挟みます。
...
The ADCMT 7461A command set is selected with `command_language`. The standard config uses `scpi` and `:READ?` for readings. USB connect only sends `*CLS`; SCPI setting commands are followed by an error check.
