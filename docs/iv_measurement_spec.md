...
Device Disconnect は該当 device を local に戻して VISA resource を close します。GPIB device の場合、REN release が同じ board 上の装置全体に効くため、同じ GPIB board で接続中の device はまとめて close してから GPIB board の REN release を試します。Source を含む board を閉じる場合は、close 前に output off と level zero を試します。ResourceManager は共有されることがあるため、個別 device の disconnect では閉じません。
...
Device Disconnect returns the affected devices to local and closes their VISA
resources. For GPIB devices, REN release affects the whole board, so disconnect
closes every connected device on the same GPIB board before attempting final
board-level REN release. If the affected board includes a source, output off and
