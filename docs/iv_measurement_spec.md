...
Device Disconnect は該当 device を local に戻して VISA resource を close します。ResourceManager は共有されることがあるため、個別 device の disconnect では閉じません。
...
Because a ResourceManager may be shared, individual device disconnect does not
